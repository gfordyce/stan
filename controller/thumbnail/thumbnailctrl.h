...
public:
    ...
    /// Draw the background
    virtual bool DrawBackground(wxDC& dc, wxThumbnailCtrl* ctrl, const wxRect& rect, int style, int index) ;
};

/*!
 * wxStanThumbnailItem class declaration
 * This item renders a scaled stan frame.
 */

class wxStanThumbnailItem: public wxThumbnailItem
{
    DECLARE_CLASS(wxStanThumbnailItem)
public:

	wxStanThumbnailItem(stan::frame* fr) :
		frame_(fr)
		{}
