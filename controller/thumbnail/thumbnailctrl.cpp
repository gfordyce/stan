...
void wxThumbnailCtrl::CalculateOverallThumbnailSize()
{
	...
	m_thumbnailOverallSize.x = m_thumbnailImageSize.x;
...
bool wxThumbnailCtrl::DrawItemBackground(int n, wxDC& dc, const wxRect& rect, int style)
{
    wxThumbnailItem* item = GetItem(n);
    if (item)
    {
...
bool wxThumbnailItem::DrawBackground(wxDC& dc, wxThumbnailCtrl* ctrl, const wxRect& rect, int style, int WXUNUSED(index))
{
...
IMPLEMENT_CLASS(wxStanThumbnailItem, wxThumbnailItem)

/// Draw the item
bool wxStanThumbnailItem::Draw(wxDC& dc, wxThumbnailCtrl* WXUNUSED(ctrl), const wxRect& rect, int WXUNUSED(style))
{
